...
export class DataService {
  ...
  constructor(private httpClient: HttpClient) {
    this.url = "https://dj.jundk-hosting.de/api";
  }

  postAddSongToList(song_name: string, interpret: string, bild_url: string, artistID: string) {
    let endPoints = "/veranstaltung/addSong"
     this.httpClient.post<any>(this.url + endPoints, {
      song_name: song_name, interpret: interpret, votinganzahl: 1, song_gespielt: 0, bild_url: bild_url, artistID: artistID
    }).subscribe(data => {
      this.postId = data.id;
    })
  }
  ...
  public deleteHistory() {
    let endPoints = "/veranstaltung/delete_history";
    this.httpClient.delete(this.url + endPoints).subscribe(data => {
      console.log(data);
    });
  }
  ...
  public getSpotifySearchResults(searchString: String): Array<Spotify> {
    ...
    this.httpClient.get<SearchResultsSpotify>(this.url + endPoints).subscribe(data => {
      ...
      for (let i = 0; i < data.response.length; i++) {
        let newSpotifyobject = new Spotify(data.response[i].name, data.response[i].artists[0].name, data.response[i].album.images[0].url, data.response[i].artists[0].id);
...
export class Spotify {
  ...
  url: string = "";
  artistID: string ="";
  constructor(song_name: string, artist: string, url: string, artistID: string) {
...
export class QueueListObject {
  ...
  constructor(song_id: number, song_name: string, interpret: string, timestamp: string, votinganzahl: number, song_gespielt: number, url: string, artistID: string) {
